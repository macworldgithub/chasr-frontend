  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 space-y-6 sm:space-y-8 max-w-7xl mx-auto w-full">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-xl sm:text-2xl font-extrabold text-white tracking-tight">Sync Operations & Audit Center</h1>
      ...
      <div className="glass-panel p-4 rounded-2xl border border-dark-border flex flex-col md:flex-row items-stretch md:items-center justify-between gap-3 sm:gap-4">
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2.5 sm:gap-3 w-full md:w-auto">
          ...
          <select
            ...
            className="w-full sm:w-auto px-3.5 py-2 rounded-xl bg-dark-card border border-dark-border text-xs text-slate-200 focus:outline-none focus:border-cyan-500 capitalize"
          >
              ...
              <table className="min-w-full text-left text-xs">
                <thead>
                  <tr className="border-b border-dark-border text-slate-400 font-mono uppercase tracking-wider text-[11px]">
                    <th className="pb-3 px-3 sm:px-4 whitespace-nowrap">Log ID</th>
                    <th className="pb-3 px-3 sm:px-4 whitespace-nowrap">Provider</th>
                    <th className="pb-3 px-3 sm:px-4 whitespace-nowrap">Status</th>
                    <th className="pb-3 px-3 sm:px-4 whitespace-nowrap">Synced Records</th>
                    <th className="pb-3 px-3 sm:px-4 whitespace-nowrap">Executed At</th>
                    <th className="pb-3 px-3 sm:px-4 text-right whitespace-nowrap">Inspector</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-dark-border/60 text-slate-200 font-sans">
                  {filteredLogs.map((log) => (
                    <tr key={log._id} className="hover:bg-dark-hover/50 transition-colors">
                      <td className="py-3.5 sm:py-4 px-3 sm:px-4 font-mono text-cyan-300 whitespace-nowrap">{log._id.slice(0, 12)}...</td>
                      <td className="py-3.5 sm:py-4 px-3 sm:px-4 font-semibold capitalize whitespace-nowrap">{log.provider}</td>
                      ...
                      <td className="py-3.5 sm:py-4 px-3 sm:px-4 font-mono text-cyan-400 whitespace-nowrap">
      ...
      {inspectLog && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/75 backdrop-blur-sm p-3 sm:p-4">
          <div className="glass-panel border border-dark-border rounded-2xl w-full max-w-2xl p-4 sm:p-6 shadow-glass relative space-y-4 max-h-[85vh] flex flex-col">
            <div className="flex items-center justify-between border-b border-dark-border pb-3">
              <div className="flex items-center gap-2 min-w-0 pr-2">
                <FileCode className="w-4 h-4 text-cyan-400 shrink-0" />
                <h3 className="font-bold text-xs sm:text-sm text-white font-mono truncate">
                  Sync Record #{inspectLog._id}
                </h3>
              </div>
              <button
                onClick={() => setInspectLog(null)}
                className="text-slate-400 hover:text-white p-1 rounded-lg hover:bg-dark-hover shrink-0"
              >
                <X className="w-4 h-4" />
              </button>
            </div>

            <div className="flex-1 overflow-y-auto bg-dark-bg p-3 sm:p-4 rounded-xl border border-dark-border text-xs font-mono text-cyan-300">
              <pre className="whitespace-pre-wrap break-all">{JSON.stringify(inspectLog, null, 2)}</pre>
            </div>
          </div>
        </div>
      )}
