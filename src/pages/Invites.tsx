  ...
  return (
    <div className="p-8 space-y-8 max-w-6xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-extrabold text-white tracking-tight">Client & Bookkeeper Invites</h1>
      ...
      <div className="glass-panel p-6 rounded-2xl border border-dark-border">
        ...
        <form onSubmit={handleCreateInvite} className="grid grid-cols-1 sm:grid-cols-3 gap-4 items-end">
          ...
          <button
            type="submit"
            disabled={isCreating}
            className="w-full py-2.5 rounded-xl bg-gradient-to-r from-brand-600 to-cyan-500 text-white font-semibold text-xs hover:opacity-90 transition-all shadow-glow-cyan flex items-center justify-center gap-2 disabled:opacity-50"
          >
            ...
            <table className="w-full text-left text-xs">
              <thead>
                <tr className="border-b border-dark-border text-slate-400 font-mono uppercase tracking-wider text-[11px]">
                  ...
                  <th className="pb-3 px-4">Invitee Email</th>
              ...
              <tbody className="divide-y divide-dark-border/60 text-slate-200 font-sans">
                {invites.map((inv) => (
                  <tr key={inv._id} className="hover:bg-dark-hover/50 transition-colors">
                    <td className="py-4 px-4 font-semibold capitalize">{inv.provider}</td>
                    ...
                    <td className="py-4 px-4 text-right space-x-2">
                      <button
                        ...
                        className="px-3 py-1.5 rounded-lg bg-dark-card border border-dark-border text-slate-200 hover:text-cyan-400 text-[11px] font-semibold transition-all inline-flex items-center gap-1"
                      >
                        {copiedToken === inv.token ? (
                          ...
                          <>
                            <Copy className="w-3 h-3 text-cyan-400" /> Share Link
                          </>
                        )}
                      </button>
                      ...
                      {inv.status === 'pending' && (
                        <button
                          ...
                          className="p-1.5 rounded-lg bg-rose-500/10 border border-rose-500/20 text-rose-400 hover:text-rose-300 transition-all"
                          title="Revoke Token"
                        >
