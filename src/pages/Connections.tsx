...
import { AccountingConnection, Provider, OrganisationInfo } from '../types';
...
export const Connections: React.FC = () => {
  ...
  const [selectedConnection, setSelectedConnection] = useState<AccountingConnection | null>(null);
  const [orgInfoMap, setOrgInfoMap] = useState<Record<string, OrganisationInfo>>({});
  const [isLoading, setIsLoading] = useState(true);
  const [syncingMap, setSyncingMap] = useState<Record<string, boolean>>({});

  // Modals state
  const [xeroTenantModalOpen, setXeroTenantModalOpen] = useState(false);
  const [xeroTenants, setXeroTenants] = useState<Array<{ id: string; name: string }>>([]);
  const [xeroConnIdForTenant, setXeroConnIdForTenant] = useState<string>('');

  const [credentialModalOpen, setCredentialModalOpen] = useState(false);
  const [credentialProvider, setCredentialProvider] = useState<Provider>('xero');
  ...
  const handleTriggerSync = async (connId: string) => {
    ...
    try {
      ...
      alert(`Sync Trigger Error: ${err.response?.data?.message || err.message}`);
      setSyncingMap((prev) => ({ ...prev, [connId]: false }));
    }
  };
  ...
  const handleOpenTenantSelector = async (connId: string) => {
    try {
      ...
      alert(`Tenant fetch error: ${err.response?.data?.message || err.message}`);
    }
  };

  const providersConfig: Array<{ name: string; provider: Provider; color: string; desc: string }> = [
    {
      name: 'Xero',
      provider: 'xero',
  ...
  return (
    <div className="p-8 space-y-8 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-extrabold text-white tracking-tight">Accounting Connections</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
        {providersConfig.map((item) => {
          const isConnected = connections.some((c) => c.provider === item.provider);
          return (
            <div
              key={item.provider}
            ...
            >
              <div>
                <div className={`w-12 h-12 rounded-2xl bg-gradient-to-tr ${item.color} flex items-center justify-center text-white font-extrabold text-lg mb-4 shadow-lg`}>
                  {item.name[0]}
                </div>
      ...
      <div className="glass-panel p-6 rounded-2xl border border-dark-border space-y-4">
        ...
        {isLoading ? (
          ...
          <div className="py-12 text-center text-xs text-slate-400 font-mono border border-dashed border-dark-border rounded-xl">
            No active connections configured. Click an OAuth or Credential button above to connect your accounting ledger.
          </div>
        ) : (
            ...
            <table className="w-full text-left text-xs">
              ...
              <tbody className="divide-y divide-dark-border/60 text-slate-200 font-sans">
                {connections.map((conn) => {
                  ...
                  return (
                    <tr key={conn._id} className="hover:bg-dark-hover/50 transition-colors">
                      ...
                      <td className="py-4 px-4 font-mono text-slate-400">{conn.authMethod}</td>
                      <td className="py-4 px-4 font-mono">
                        {org ? (
                          <div className="flex items-center gap-1.5 text-cyan-300 font-semibold">
                            <Building className="w-3.5 h-3.5" />
                            {org.name}
                            {org.currencyCode && <span className="text-[10px] text-slate-400">({org.currencyCode})</span>}
                          </div>
                        ) : conn.activeTenantId ? (
                          <span className="text-slate-300">Tenant: {conn.activeTenantId}</span>
                        ) : (
                          <span className="text-slate-500 italic">No tenant assigned</span>
                        )}
                      </td>
